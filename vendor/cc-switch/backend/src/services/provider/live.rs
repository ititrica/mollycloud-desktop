...
pub(crate) fn build_effective_settings_with_common_config(
    ...
    crate::isolation::isolate_live_settings(app_type, &mut effective_settings)?;
...
pub(crate) fn build_effective_provider_for_live_with_codex_oauth_manager(
    ...
    crate::isolation::isolate_live_settings(app_type, &mut effective_provider.settings_config)?;
...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    let mut isolated_provider = provider.clone();
    crate::isolation::isolate_live_settings(app_type, &mut isolated_provider.settings_config)?;
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    // MOLLY_CCSWITCH_PROXY_MANAGED 占位符和本地代理地址，不是用户的真实配置。一旦导入，
