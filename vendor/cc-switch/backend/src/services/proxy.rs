...
const PROXY_TOKEN_PLACEHOLDER: &str = "MOLLY_CCSWITCH_PROXY_MANAGED";
...
impl ProxyService {
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            crate::proxy_ownership::require_no_external_proxy(&app).map_err(|e| e.to_string())?;
            crate::proxy_ownership::backup_original_for_app(&app).map_err(|e| e.to_string())?;
            // 1) 代理服务未运行则自动启动
        ...
        // 留下接管时写入的占位符（代理地址/MOLLY_CCSWITCH_PROXY_MANAGED token），客户端无法工作。
    ...
    async fn backup_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(config) = self.read_claude_live() {
            ...
            // 即便走到备份路径也会把代理占位符再写回 Live，永久卡在 127.0.0.1:24327。
        ...
        if let Ok(mut config) = self.read_codex_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Codex, &config) {
                ...
                crate::proxy_ownership::record_takeover_baseline(&AppType::Codex).map_err(|e| e.to_string())?;
            }
        }
        ...
        if let Ok(config) = self.read_gemini_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Gemini, &config) {
                ...
                crate::proxy_ownership::record_takeover_baseline(&AppType::Gemini).map_err(|e| e.to_string())?;
            }
        }
    ...
    async fn backup_live_config_strict(&self, app_type: &AppType) -> Result<(), String> {
        ...
        crate::proxy_ownership::record_takeover_baseline(app_type).map_err(|e| e.to_string())?;
    ...
    pub(crate) async fn restore_live_config_for_app_with_fallback_inner(
        ...
        // A private backup alone is insufficient proof of ownership.
        if !self.detect_takeover_in_live_config_for_app(app_type) {
            return Ok(());
        }
        crate::proxy_ownership::require_unchanged(app_type)?;
        let app_type_str = app_type.as_str();
        ...
        if let Some(backup) = backup {
            ...
            // Live 永远卡在 127.0.0.1:24327。落到下面的 SSOT 兜底重建。
            if Self::live_has_proxy_placeholder_for_app(app_type, &config) {
                log::warn!(
...
mod tests {
    ...
    fn normal_claude_takeover_without_token_keeps_auth_token_fallback() {
        ...
        ProxyService::apply_claude_takeover_fields(&mut live_config, "http://127.0.0.1:24327");
...
[model_providers.rightcode]
name = "RightCode"
base_url = "http://127.0.0.1:24327/v1"
wire_api = "responses"
"#
        });
...
base_url = "https://api.deepseek.com/v1"
wire_api = "responses"
experimental_bearer_token = "MOLLY_CCSWITCH_PROXY_MANAGED"
"#;
...
"#,
        ...
        assert!(
            !live_config.contains("http://127.0.0.1:24327"),
...
"#
        ...
        assert!(live_config.contains("base_url = \"http://127.0.0.1:24327/v1\""));
        assert!(
            !crate::codex_config::get_codex_auth_path().exists(),
...
"#;
    ...
    fn codex_takeover_without_provider_selects_a_local_authenticated_route() {
        for input in [
            ...
            let url = "http://127.0.0.1:24327/v1";
...
"#;
    ...
    async fn hot_switch_provider_updates_claude_live_while_preserving_takeover_fields() {
        ...
        service
            .write_claude_live(&json!({
                "env": {
                    "ANTHROPIC_BASE_URL": "http://127.0.0.1:24327",
...
"#,
        ...
        std::fs::write(
            crate::codex_config::get_codex_auth_path(),
            r#"{"OPENAI_API_KEY":"MOLLY_CCSWITCH_PROXY_MANAGED"}"#,
        )
        ...
        std::fs::write(&gemini_env_path, "GEMINI_API_KEY=MOLLY_CCSWITCH_PROXY_MANAGED\n")
    ...
    async fn failed_hot_switch_grokbuild_keeps_previous_current_and_backup() {
        ...
        let takeover = crate::grok_config::apply_proxy_takeover(
            provider_a.settings_config["config"]
                .as_str()
                .expect("provider config"),
            "http://127.0.0.1:24327/grokbuild/v1",
            PROXY_TOKEN_PLACEHOLDER,
        )
