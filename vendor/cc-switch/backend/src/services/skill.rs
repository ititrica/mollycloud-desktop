...
impl SkillService {
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        // 默认路径：回退到用户主目录下的标准位置。
        // 必须走 get_home_dir()（可被 CC_SWITCH_TEST_HOME 覆盖）：Windows 上 Some(crate::config::get_home_dir())
    ...
    async fn download_repo(&self, repo: &SkillRepo) -> Result<(tempfile::TempDir, String)> {
        ...
        let temp_dir = tempfile::tempdir_in(crate::embedded::private_root().join("cache"))?;
    ...
    fn extract_local_zip(zip_path: &Path) -> Result<tempfile::TempDir> {
        Self::extract_local_zip_in(zip_path, &crate::embedded::private_root().join("cache"))
    }
...
mod tests {
    ...
    fn get_app_skills_dir_honors_test_home_override() {
        // 回归：曾直呼 Some(crate::config::get_home_dir()) 绕过 CC_SWITCH_TEST_HOME——Unix 上碰巧跟 $HOME
