...
pub fn get_grok_config_dir() -> PathBuf {
    crate::settings::get_grok_override_dir().or_else(|| crate::config::cli_env_path("GROK_HOME")).unwrap_or_else(|| get_home_dir().join(".grok"))
}
...
"#
    ...
    fn extracts_selected_model_and_updates_takeover_fields() {
        ...
        let updated = apply_proxy_takeover(
            ...
            "MOLLY_CCSWITCH_PROXY_MANAGED",
        )
        .expect("takeover config");
        let selected = extract_model_config(&updated).expect("updated selected model");
        assert_eq!(selected.base_url, "http://127.0.0.1:24327/grokbuild/v1");
        assert_eq!(selected.api_key.as_deref(), Some("MOLLY_CCSWITCH_PROXY_MANAGED"));
        assert!(has_proxy_placeholder(&updated, "MOLLY_CCSWITCH_PROXY_MANAGED"));
    }
