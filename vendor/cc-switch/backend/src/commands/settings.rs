...
pub async fn save_settings(
    ...
    let targets_changed = merged.claude_config_dir != existing.claude_config_dir
        || merged.codex_config_dir != existing.codex_config_dir
        || merged.gemini_config_dir != existing.gemini_config_dir
        || merged.grok_config_dir != existing.grok_config_dir;
    if targets_changed && state.proxy_service.detect_takeover_in_live_configs() {
        return Err("请先关闭本地代理接管，再更改工具配置目录，以便将原目录正确恢复。".into());
    }
...
// Application lifecycle belongs exclusively to MollyCloud.
#[tauri::command]
pub async fn restart_app(_: AppHandle) -> Result<bool, String> {
    Err("内置 CC Switch 随 MollyCloud 更新和重启。".into())
}
#[tauri::command]
pub async fn install_update_and_restart(_: AppHandle) -> Result<bool, String> {
    Err("内置 CC Switch 随 MollyCloud 更新和重启。".into())
}
#[tauri::command]
pub async fn check_app_update_available(_: AppHandle) -> Result<Option<String>, String> { Ok(None) }
