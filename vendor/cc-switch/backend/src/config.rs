...
/// CLI configuration follows the actual user profile. Only the manager's own
/// database, settings and backups live under MollyCloud's application data.
pub fn get_home_dir() -> PathBuf {
    #[cfg(any(test, feature = "test-hooks"))]
    if let Some(home) = crate::embedded::test_home() {
        return home;
    }
    #[cfg(test)]
    panic!("filesystem tests must supply a disposable CC Switch home");
    #[cfg(not(test))]
    dirs::home_dir().expect("the operating system must provide a user profile")
}

/// Tests deliberately ignore the developer's inherited CLI path overrides.
pub(crate) fn cli_env_path(name: &str) -> Option<PathBuf> {
    #[cfg(any(test, feature = "test-hooks"))]
    if crate::embedded::test_home().is_some() { return None; }
...
pub fn get_claude_config_dir() -> PathBuf {
    ...
    cli_env_path("CLAUDE_CONFIG_DIR").unwrap_or_else(|| get_home_dir().join(".claude"))
}

/// 默认 Claude MCP 配置文件路径 (~/.claude.json)
pub fn get_default_claude_mcp_path() -> PathBuf {
    cli_env_path("CLAUDE_CONFIG_DIR").map(|dir| dir.join(".claude.json"))
        .unwrap_or_else(|| get_home_dir().join(".claude.json"))
}
...
fn default_mcp_path_for_config_dir(dir: &Path) -> Option<PathBuf> {
    ...
    if path_eq_lexical(dir, &default_config_dir) {
        return Some(get_home_dir().join(".claude.json"));
    }
...
pub fn get_app_config_dir() -> PathBuf {
    crate::embedded::private_root().join("data")
}
...
fn atomic_write_with_unix_mode(
    ...
    crate::embedded::require_config_path(path)?;
    crate::proxy_ownership::backup_original_file(path)?;
    #[cfg(not(unix))]
    let _ = unix_mode;
    ...
    crate::proxy_ownership::record_write(path, data)?;
    Ok(())
}
...
pub fn copy_file(from: &Path, to: &Path) -> Result<(), AppError> {
    crate::embedded::require_config_path(from)?;
    crate::embedded::require_config_path(to)?;
...
pub fn delete_file(path: &Path) -> Result<(), AppError> {
    ...
    crate::proxy_ownership::record_removal(path)?;
    Ok(())
}
