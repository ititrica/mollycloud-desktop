...
pub const CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "molly-ccswitch-official";
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "cc-switch-model-catalog.json";
const CODEX_PROXY_AUTH_PLACEHOLDER: &str = "MOLLY_CCSWITCH_PROXY_MANAGED";
...
pub fn get_codex_config_dir() -> PathBuf {
    ...
    crate::config::cli_env_path("CODEX_HOME").unwrap_or_else(|| get_home_dir().join(".codex"))
}
...
fn extract_codex_managed_oauth_account_id(auth: &Value) -> Option<String> {
    ...
    let api_key_is_clearable = auth
        .get("OPENAI_API_KEY")
        .is_none_or(|value| value.is_null() || value.as_str() == Some("MOLLY_CCSWITCH_PROXY_MANAGED"));
...
fn load_codex_model_template_from_bundled() -> Result<Option<Value>, AppError> {
    // Filesystem regressions never execute a developer's installed CLI.
    #[cfg(any(test, feature = "test-hooks"))]
    if crate::embedded::test_home().is_some() { return Ok(None); }
    for candidate in codex_cli_candidates() {
        let output = match codex_bundled_models_command(&candidate).output() {
            Ok(output) if output.status.success() => output,
            _ => continue,
        };
        let Ok(catalog) = serde_json::from_slice::<Value>(&output.stdout) else { continue; };
        if let Some(template) = find_codex_model_template(&catalog) { return Ok(Some(template)); }
    }
    Ok(None)
}
...
command = "example"
"#;
        let output = apply_codex_official_proxy_route(input, "http://127.0.0.1:24327/v1")
    ...
    fn official_proxy_route_rejects_non_table_model_providers_without_panicking() {
        for input in [
            ...
            let result = apply_codex_official_proxy_route(input, "http://127.0.0.1:24327/v1");
...
model_providers = { rightcode = { name = "RightCode", experimental_bearer_token = "MOLLY_CCSWITCH_PROXY_MANAGED" } }
"#;
        let projected = apply_codex_official_proxy_route(input, "http://127.0.0.1:24327/v1")
