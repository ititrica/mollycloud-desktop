...
impl AppSettings {
    fn settings_path() -> Option<PathBuf> {
        ...
        Some(
            crate::embedded::private_root().join("home")
                .join(".cc-switch")
                .join("settings.json"),
        )
    }
...
fn settings_store() -> &'static RwLock<AppSettings> {
    SETTINGS_STORE.get_or_init(|| RwLock::new(AppSettings::load_from_file().unwrap_or_else(|error| {
        log::warn!("拒绝读取非私有内置设置: {error}");
        AppSettings::default()
    })))
}
...
pub(crate) fn resolve_override_path(raw: &str) -> PathBuf {
    ...
    if raw == "~" {
        if let Some(home) = Some(crate::config::get_home_dir()) {
            return home;
        }
...
pub fn update_settings(mut new_settings: AppSettings) -> Result<(), AppError> {
    ...
    crate::embedded::validate_settings(&new_settings)?;
...
mod tests {
    ...
    fn override_paths_expand_windows_style_tilde_separators() {
        let home = Some(crate::config::get_home_dir()).expect("home directory");
        assert_eq!(
