...
const PROXY_AUTH_PLACEHOLDER: &str = "MOLLY_CCSWITCH_PROXY_MANAGED";
...
fn reject_proxy_placeholder_for_managed_account_upstream(
    ...
    Err(ProxyError::AuthError(
        "Managed account proxy auth was not resolved; MOLLY_CCSWITCH_PROXY_MANAGED must not be sent upstream"
            .to_string(),
    ))
}
...
mod tests {
    ...
    fn managed_account_upstream_rejects_proxy_managed_placeholder_header() {
        ...
        headers.insert(
            "authorization",
            HeaderValue::from_static("Bearer MOLLY_CCSWITCH_PROXY_MANAGED"),
        );
        ...
        assert!(matches!(
            err,
            ProxyError::AuthError(message) if message.contains("MOLLY_CCSWITCH_PROXY_MANAGED")
        ));
