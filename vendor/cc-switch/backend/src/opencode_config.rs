...
pub fn get_opencode_dir() -> PathBuf {
    ...
    crate::config::cli_env_path("OPENCODE_CONFIG_DIR").unwrap_or_else(||
        crate::config::cli_env_path("XDG_CONFIG_HOME").unwrap_or_else(|| crate::config::get_home_dir().join(".config")).join("opencode"))
}
...
pub fn get_opencode_db_path() -> PathBuf {
    crate::config::cli_env_path("OPENCODE_DB").unwrap_or_else(|| get_opencode_data_dir().join("opencode.db"))
}

fn get_opencode_data_dir() -> PathBuf {
    // OpenCode 使用 xdg-basedir，不遵守 macOS/Windows 平台约定，
    // 所有平台默认都落在 ~/.local/share/opencode
    crate::config::cli_env_path("XDG_DATA_HOME").unwrap_or_else(||
        crate::config::get_home_dir().join(".local").join("share")).join("opencode")
}
