...
// Window lifecycle and manager storage remain owned by MollyCloud.
const HOST_OWNS_LIFECYCLE = true;
              ...
              <TabsContent value="general" className="space-y-6 mt-0">
                {settings ? (
                  ...
                  >
                    ...
                    />
                    <ThemeSettings />
                    <section className="rounded-lg border border-border p-4 text-sm space-y-2"><h3 className="font-medium">本机工具配置目录</h3><p className="text-muted-foreground">供应商库由 MollyCloud 独立保存；供应商、MCP 和 Skills 会写入本机工具实际使用的目录。</p><p className="break-all font-mono text-xs">{resolvedDirs.appConfig}</p><p className="break-all font-mono text-xs">Claude Code：{resolvedDirs.claude}</p><p className="break-all font-mono text-xs">Codex：{resolvedDirs.codex}</p><p className="break-all font-mono text-xs">Gemini：{resolvedDirs.gemini}</p></section>
                    ...
                    <CodexAuthSettings
                      ...
                      onChange={handleAutoSave}
                    />
                    {!HOST_OWNS_LIFECYCLE && (<WindowSettings
                      settings={settings}
                      onChange={handleAutoSave}
                    />)}
                    {(<TerminalSettings
