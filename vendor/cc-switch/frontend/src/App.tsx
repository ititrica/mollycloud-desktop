...
const EMBEDDED_APPS: readonly AppId[] = APP_IDS;
const getInitialApp = (): AppId => {
  const saved = localStorage.getItem(STORAGE_KEY) as AppId | null;
  if (saved && EMBEDDED_APPS.includes(saved)) {
    return saved;
  }
  return "codex";
};
...
function App() {
  ...
  const [pendingProvider, setPendingProvider] = useState<{ id: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => selectProvider());
    const selectProvider = () => {
      const card = document.querySelector(`[data-provider-id="${CSS.escape(pendingProvider.id)}"]`);
      if (!card) return;
      document.querySelectorAll("[data-molly-selected]").forEach((node) => node.removeAttribute("data-molly-selected"));
      card.setAttribute("data-molly-selected", "true");
  ...
  const handleOpenTerminal = async (provider: Provider) => {
    try {
      ...
      toast.success("已启动所选工具，使用当前本机配置");
    } catch (error) {
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <ProviderList
                      ...
                      onOpenTerminal={
                        ["claude", "codex", "gemini"].includes(activeApp) ? handleOpenTerminal : undefined
                      }
