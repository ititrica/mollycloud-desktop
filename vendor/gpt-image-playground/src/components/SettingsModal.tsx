...
import { mollyEmbedded, saveMollySettings } from '../lib/mollyBridge'
            ...
            {activeTab === 'api' && (
              ...
              <div className="block">
                ...
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                  {mollyEmbedded ? '手动填写密钥后会保存到当前账户，重启后仍可使用；清除应用数据前请先导出备份。' : <>支持通过查询参数覆盖：<code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">?apiKey=</code></>}
                </div>
                {mollyEmbedded && <button type="button" className="mt-2 rounded-lg bg-blue-500 px-3 py-2 text-sm text-white hover:bg-blue-600" onClick={async () => {
                  commitActiveProfilePatch({ apiKey: activeProfile.apiKey })
                  try { await saveMollySettings(); showToast('密钥已保存，重启后仍可使用', 'success') }
                  catch { showToast('密钥保存失败，请检查本地存储后重试', 'error') }
                }}>保存密钥</button>}
              </div>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl bg-gray-50/80 p-4 border border-gray-200/60 dark:bg-white/[0.02] dark:border-white/[0.05] flex items-start gap-3">
                  ...
                  <div className="text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                    {mollyEmbedded ? '配置、任务和图片保存在 MollyCloud 当前账户的本地工作台中。重启和更新会保留数据；清除应用数据或更换设备前，请先导出备份。生成时会把提示词与参考图发送给你配置的服务商。' : '所有的配置、任务和生成的图片均仅保存在您的浏览器本地（除非您使用的服务商存储了它们）。如果您需要清理浏览器站点数据、重置浏览器或使用其他设备，请先导出备份。'}
                  </div>
                </div>
