...
import { mollyDb, mollyEmbedded } from './mollyBridge'
...
function dbTransaction<T>(
  ...
  if (mollyEmbedded) {
    // 保留上游操作入口，存储由父窗口固定到当前账户的四个专用表。
    const store = Object.fromEntries(['get', 'getAll', 'getAllKeys', 'put', 'delete', 'clear'].map((method) => [
      method, (value?: unknown) => ({ store: storeName, method, value }),
    ])) as unknown as IDBObjectStore
    return mollyDb<T>([fn(store) as unknown as import('./mollyBridge').DbOperation])
  }
  return openDB().then(
    (db) =>
...
export function commitTaskDeletion(deletedTaskIds: string[], updatedTasks: TaskRecord[], updatedConversations: AgentConversation[]): Promise<undefined> {
  if (mollyEmbedded) return mollyDb([
    ...deletedTaskIds.map((value) => ({ store: STORE_TASKS, method: 'delete' as const, value })),
    ...updatedTasks.map((value) => ({ store: STORE_TASKS, method: 'put' as const, value })),
    ...updatedConversations.map((value) => ({ store: STORE_AGENT_CONVERSATIONS, method: 'put' as const, value })),
  ])
  return openDB().then(
    (db) =>
      new Promise((resolve, reject) => {
...
export function replaceAgentConversations(conversations: AgentConversation[]): Promise<undefined> {
  if (mollyEmbedded) return mollyDb([
    { store: STORE_AGENT_CONVERSATIONS, method: 'clear' },
    ...conversations.map((value) => ({ store: STORE_AGENT_CONVERSATIONS, method: 'put' as const, value })),
  ])
  return openDB().then(
    (db) =>
      new Promise((resolve, reject) => {
...
export function deleteImage(id: string): Promise<undefined> {
  if (mollyEmbedded) return mollyDb([STORE_IMAGES, STORE_THUMBNAILS].map((store) => ({ store, method: 'delete', value: id })))
  return openDB().then(
    (db) =>
      new Promise((resolve, reject) => {
...
export function clearImages(): Promise<undefined> {
  if (mollyEmbedded) return mollyDb([STORE_IMAGES, STORE_THUMBNAILS].map((store) => ({ store, method: 'clear' })))
  return openDB().then(
    (db) =>
      new Promise((resolve, reject) => {
        const tx = db.transaction([STORE_IMAGES, STORE_THUMBNAILS], 'readwrite')
