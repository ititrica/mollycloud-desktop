...
import { mollyEmbedded, notifyMolly } from './lib/mollyBridge'
...
export default function App() {
  ...
  useEffect(() => {
    if (!mollyEmbedded) return
    const storageError = () => useStore.getState().showToast('配置保存失败，请检查本地存储空间后重新保存。', 'error')
    window.addEventListener('molly-storage-error', storageError)
    const unsubscribe = useStore.subscribe((state, previous) => {
      if (state.tasks.some((task) => task.status !== 'running' && previous.tasks.find((old) => old.id === task.id)?.status === 'running')) notifyMolly('settled')
    })
    return () => { unsubscribe(); window.removeEventListener('molly-storage-error', storageError) }
  }, [])
