...
use assistant_config::{get_assistant_config, save_assistant_config};
...
fn set_api_key(app: AppHandle, api_key: String) -> Result<(), String> {
    ...
    std::fs::write(&temporary, enc).map_err(|e| e.to_string())?;
    std::fs::rename(&temporary, &path).map_err(|e| e.to_string())
}

#[tauri::command]
fn has_api_key(app: AppHandle) -> Result<bool, String> {
    if !api_key_path(&app)?.exists() { return Ok(false); }
    get_api_key(app).map(|key| !key.trim().is_empty())
}
...
fn setup_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(app)
        .items(&[&open_console, &toggle, &separator, &restart, &quit_label])
        .build()?;
