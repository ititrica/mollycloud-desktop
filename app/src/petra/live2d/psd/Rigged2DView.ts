...
export class Rigged2DView implements PetView {
  ...
  getHeadBounds(): { left: number; top: number; right: number; bottom: number } | null {
    const corners = this.runtime.headCorners;
    if (!corners || !this.canvas.isConnected) return null;
    ...
    const transform = (x: number, y: number) => matrix.transformPoint({ x: x - originX, y: y - originY });
    const box = [transform(0, 0), transform(width, 0), transform(width, height), transform(0, height)];
    const rect = this.canvas.getBoundingClientRect();
    // 用实际画布位置校准变换后的四角，包含居中、屏幕边缘偏移、点击缩放及倒挂旋转。
    const offsetX = rect.left - Math.min(...box.map((point) => point.x));
    const offsetY = rect.top - Math.min(...box.map((point) => point.y));
    const points = [];
    for (let i = 0; i < corners.length; i += 2) {
      points.push(transform(corners[i] * width / this.runtime.canvasWidth, corners[i + 1] * height / this.runtime.canvasHeight));
    }
    return {
      left: offsetX + Math.min(...points.map((point) => point.x)),
      top: offsetY + Math.min(...points.map((point) => point.y)),
      right: offsetX + Math.max(...points.map((point) => point.x)),
      bottom: offsetY + Math.max(...points.map((point) => point.y)),
    };
  }
