...
export class PsdRuntime {
  ...
  private headProbe: Layer | null = null;
  private FS = 1;
  private NP: any = null;
  private BP: any = null;
  private FC: any = null;
  ...
  /** 头部四角（PSD 画布坐标），随面部的变形参数更新，不含长发和裙摆的外扩范围。 */
  get headCorners(): Float32Array | null {
    return this.headProbe?.cur ?? null;
  }
  /** 画布宽（PSD 尺寸） */
  ...
  async load(u8: Uint8Array): Promise<string[]> {
    ...
    const faceLayer = this.layers.find((layer) => layer.bn === "face");
    if (faceLayer) {
      const face = this.A.face;
      const sidePadding = (face.x1 - face.x0) * 0.1;
      const left = Math.max(0, face.x0 - sidePadding);
      const right = Math.min(this.CW, face.x1 + sidePadding);
      let top = face.y0;
      // 脸部锚点不含头发：只在脸部附近找可见发顶，避免长发/侧发把悬浮控件推远。
      for (const layer of rig.layers) {
        const name = Rigger.baseName(layer.name);
        if (name !== "front hair" && name !== "back hair" && name !== "headwear") continue;
        const x0 = Math.max(0, Math.ceil(left - layer.x));
        const x1 = Math.min(layer.img.width, Math.ceil(right - layer.x));
        const y1 = Math.min(layer.img.height, Math.ceil(top - layer.y));
        for (let y = 0; y < y1; y++) {
          let visible = false;
          for (let x = x0; x < x1; x++) {
            if (layer.img.data[(y * layer.img.width + x) * 4 + 3] >= 32) {
              visible = true;
              break;
            }
          }
          if (visible) {
            top = Math.min(top, layer.y + y);
            break;
          }
        }
      }
      const base = new Float32Array([left, top, right, top, right, face.y1, left, face.y1]);
      // 仅作定位探针，复用面部变形；不加入渲染层或角色碰撞边界。
      this.headProbe = { ...faceLayer, base, cur: new Float32Array(base) };
    }
  ...
  update(dt: number, overrides?: Partial<RigParams>) {
    ...
    if (this.headProbe) this.deform(this.headProbe, e);
