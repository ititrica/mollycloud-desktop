...
const HISTORY_EVENT = "petra-assistant-history-changed";
const STATE_EVENT = "petra-assistant-state-changed";
...
function saveHistory() {
  ...
  publishAssistantHistory();
}

export function getDisplayHistory(): AssistantDisplayMessage[] {
  return history
    .filter((message): message is ChatMessage & { role: "user" | "assistant"; content: string } => {
...
export function publishAssistantHistory() {
  void emitTo(CONSOLE_WINDOW, HISTORY_EVENT, { messages: getDisplayHistory() }).catch(() => undefined);
}

function publishAssistantState(isBusy: boolean, error = "") {
  void emitTo(CONSOLE_WINDOW, STATE_EVENT, { busy: isBusy, error }).catch(() => undefined);
}
