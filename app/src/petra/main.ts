...
import { modelParameters, modelAutoOptions, petDraftSchema, type PetSettingsRequest, type PetSettingsSnapshot } from "../petSettings";
...
const HIDDEN_PRODUCTION_WARNINGS = new Set([
...
const BUNDLED_MODEL = "Molly.psd";
const BUNDLED_MODEL_REVISION_KEY = "mollycloud-bundled-model-v2";
const POS_KEY = "live2d-pet-position"; // 桌宠位置持久化

// 旧版曾把 psd2live 模型或已导入模型记为优先项。首次升级时切回唯一的内置 PSD，
// 后续仍保留 Petra 原生的模型导入与切换能力。
if (localStorage.getItem(BUNDLED_MODEL_REVISION_KEY) !== BUNDLED_MODEL) {
  localStorage.removeItem(PSD_KEY);
  localStorage.setItem(BUILTIN_KEY, BUNDLED_MODEL);
  localStorage.setItem(BUNDLED_MODEL_REVISION_KEY, BUNDLED_MODEL);
}
...
interface ConsoleAssistantConfig {
  enabled: boolean;
  provider: string;
  model: string;
  persona: string;
...
async function syncAssistantSwitchToConsole(enabled: boolean) {
  try {
    const config = await invoke<ConsoleAssistantConfig>("get_assistant_config");
    await invoke("save_assistant_config", {
      config: {
        enabled,
        ...
        molly_key_id: config.molly_key_id,
      },
    });
  } catch {
    /* 控制台尚未初始化时仍保留桌宠本地设置 */
...
async function applyModelScale(s: number, record = false) {
  ...
  saveSettings(settings);
  const modelW = Math.round(300 * clamped); // 使用 Petra 原生 PSD 模型视觉基准
...
function toggleDebugBorder() {
  ...
  settings.debugBorder = debugBorderVisible;
  saveSettings(settings);
  const el = ensureModelGuide("debug-border");
...
function toggleModelBounds() {
  ...
  settings.debugModelBounds = debugModelBoundsVisible;
  saveSettings(settings);
  const el = ensureModelGuide("model-bounds");
...
async function makePsdView(bytes: Uint8Array): Promise<Rigged2DView> {
  ...
  v.warnings
    .filter((warning) => !HIDDEN_PRODUCTION_WARNINGS.has(warning))
    .forEach((warning) => toast(warning, "warn"));
  return v;
}
...
async function createView(): Promise<PetView> {
  ...
  // 唯一的内置 PSD 加载失败：不回退到旧 psd2live 模型。
  throw new Error(`模型加载失败（${BUNDLED_MODEL} 缺失或无法解析）`);
}

function ensureModelGuide(id: string): HTMLElement {
...
async function reloadView() {
  ...
  view.unmount();
  await mountView();
  if (previousModel.name !== currentModel.name || previousModel.type !== currentModel.type) resetBoundsOnModelSwitch();
  // 切换模型：恢复该模型自己的大小（无记录用默认 100%）
  const key = currentModel.name ?? "";
  const scale = settings.modelScales[key] ?? 1;
  await applyModelScale(scale, false);
}

let petSettingsQueue = Promise.resolve();

async function petSettingsSnapshot(): Promise<PetSettingsSnapshot> {
  const manifest = JSON.parse(await invoke<string>("read_model_manifest"));
  const builtins: string[] = manifest.files?.length ? manifest.files : manifest.file ? [manifest.file] : manifest.active ? [manifest.active] : [];
  const imported = await invoke<string[]>("list_models");
...
async function selectPetModel(model: PetSettingsSnapshot["draft"]["model"]) {
  const snapshot = await petSettingsSnapshot();
  if (!snapshot.models.some(item => item.name === model.name && item.type === model.type)) throw new Error("模型不存在，请重新读取列表。");
  const previousBuiltin = localStorage.getItem(BUILTIN_KEY);
  const previousImported = localStorage.getItem(PSD_KEY);
  try {
    if (model.type === "import") localStorage.setItem(PSD_KEY, model.name);
    else { localStorage.setItem(BUILTIN_KEY, model.name); localStorage.removeItem(PSD_KEY); }
    await reloadView();
    if (currentModel.name !== model.name || (model.type === "import" && currentModel.type !== "import")) throw new Error("模型加载失败，已保留原模型。");
  } catch (error) {
    if (previousBuiltin) localStorage.setItem(BUILTIN_KEY, previousBuiltin); else localStorage.removeItem(BUILTIN_KEY);
    if (previousImported) localStorage.setItem(PSD_KEY, previousImported); else localStorage.removeItem(PSD_KEY);
    await reloadView().catch(() => {});
    throw error;
  }
}

async function handlePetSettings(request: PetSettingsRequest): Promise<PetSettingsSnapshot> {
  if (request.action === "read") return petSettingsSnapshot();
  if (request.action === "select") await selectPetModel(request.model);
  else if (request.action === "delete") {
    const snapshot = await petSettingsSnapshot();
    if (!snapshot.models.some(model => model.type === "import" && model.name === request.name)) throw new Error("只能删除已导入的模型。");
    const wasCurrent = currentModel.type === "import" && currentModel.name === request.name;
    if (wasCurrent) {
      const builtin = snapshot.models.find(model => model.type === "manifest");
      if (!builtin) throw new Error("没有可切换的内置模型，无法删除当前模型。");
      await selectPetModel(builtin);
    }
    try { await invoke("delete_imported_model", { name: request.name }); }
    catch (error) { if (wasCurrent) await selectPetModel(snapshot.draft.model); throw error; }
  } else if (request.action === "clear-history") {
    clearHistory();
  } else if (request.action === "fetch-models") {
    const snapshot = await petSettingsSnapshot();
    const assistant = petDraftSchema.parse({ ...snapshot.draft, assistant: request.assistant }).assistant;
    const sameEndpoint = assistant.provider === settings.assistant.provider && assistant.customBaseUrl === settings.assistant.customBaseUrl;
    const key = request.apiKey?.trim() || (request.useSavedKey && sameEndpoint && snapshot.apiKeyConfigured ? await invoke<string>("get_api_key") : "");
    snapshot.availableModels = await listModels(assistant.provider, key, assistant.customBaseUrl);
    return snapshot;
  } else if (request.action === "save") {
    const parsed = petDraftSchema.safeParse(request.draft);
    if (!parsed.success) throw new Error(parsed.error.issues[0]?.message ?? "设置无效");
    const draft = parsed.data;
    if (draft.model.name !== currentModel.name || draft.model.type !== currentModel.type) throw new Error("模型已在其他位置切换，请重新读取设置。");
    if (request.apiKey && request.apiKey.length > 4096) throw new Error("API 密钥过长。");
    const previous = structuredClone(settings);
    const endpointChanged = draft.assistant.provider !== previous.assistant.provider || draft.assistant.customBaseUrl !== previous.assistant.customBaseUrl;
    const next: Settings = { ...settings, assistant: draft.assistant, modelScale: draft.modelScale, boundsPadding: draft.boundsPadding, debugBorder: draft.debugBorder, debugModelBounds: draft.debugModelBounds,
      modelScales: { ...settings.modelScales, [draft.model.name]: draft.modelScale },
      modelParams: { ...settings.modelParams, [draft.model.name]: draft.params }, modelAuto: { ...settings.modelAuto, [draft.model.name]: draft.auto } };
    ...
    clearApiKeyCache();
    await applyModelScale(draft.modelScale, true);
    (view as Rigged2DView).setBoundsPadding?.(draft.boundsPadding);
    if (view instanceof Rigged2DView) {
      for (const [key, value] of Object.entries(draft.params)) view.setParam(key, value);
      for (const opt of modelAutoOptions) view.setAutoOption(opt.key, draft.auto[opt.key] ?? true);
    }
...
async function boot() {
  ...
  void listen<ConsoleAssistantConfig>("assistant-config-changed", (event) => {
  ...
  void listen<{ text?: string }>("petra-assistant-send", (event) => {
  ...
  await listen<{ id: string; request: PetSettingsRequest }>("petra-settings-request", event => {
    const { id, request } = event.payload;
    if (typeof id !== "string" || id.length > 80) return;
    // Serialize mutations so model switching and saving cannot race each other.
    petSettingsQueue = petSettingsQueue.then(async () => {
      try { await emitTo("console", "petra-settings-response", { id, value: await handlePetSettings(request) }); }
      catch (error) { await emitTo("console", "petra-settings-response", { id, error: error instanceof Error ? error.message : String(error) }); }
    }).catch(() => {});
  });
  engine.setScale(scaleFactor);
  ...
  if (settings.debugBorder) toggleDebugBorder();
  if (settings.debugModelBounds) toggleModelBounds();
  // 小助手对话期间桌宠静止，关闭后恢复漫游
  setLifecycle(
  ...
  // 左键：按住可拖动桌宠；轻点（<6px 未拖）仅触发"摸头"反应，不自动展开信息或助手面板。
...
// ---------- 主循环 ----------
  ...
  app.app.ticker.add(() => {
    ...
    const cb = view.getCharacterBounds?.() ?? null;
    engine.syncModelOffset(cb ?? undefined);
    if (balancePill.isOpen) engine.suspend(1000);
    ...
    const head = view.getHeadBounds?.() ?? {
      left: mr.left + mr.width * 0.2,
      top: mr.top,
      right: mr.right - mr.width * 0.2,
      bottom: mr.top + mr.height * 0.35,
    };
    balancePill.position(head, vr);
  });
}
...
function buildMenu(engine: BehaviorEngine) {
  return [
...

    {
      ...
      onPick: () => {
        ...
        saveSettings(settings);
        void syncAssistantSwitchToConsole(settings.assistant.enabled);
  ...
  ].filter(item => !["model", "assistant-settings", "autostart"].includes(item.id));
}
...
function toggleChatHistory() {
  ...
  // 与控制台聊天窗口共用同一份过滤后的 Petra 对话记录。
  const msgs = getDisplayHistory();
...
async function checkUpdate(manual = false) {
  // MollyCloud 尚未配置自己的签名更新源。保留 Petra 更新模块，开发阶段不连接 Petra 的发行源，
  // 避免未来误把 MollyCloud 客户端更新成上游 Petra。
  const updaterConfigured = false;
  if (!updaterConfigured) {
    if (manual) toast("MollyCloud 开发版暂未配置自动更新源");
    return;
  }
